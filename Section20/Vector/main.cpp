...
//Use for_each and a lambda expression to display vector elenets
...
void test6() {
    ...
    while (it != vec.end()) {
        if(*it % 2 == 0)
            ...
            it++;   //only increment if no erased!
    }
    display(vec);
}
...
void test9() {
    //std::back_inserter consctructs a back-insert iterator that insterts new elements
    //at the end of the contaioner it applied to. It's a special output iterator
    //It's awesome!!! and bery efficient
    //There is also a fron_inserter we can use with deques and lists
    //Copy on list to another using an iterator and back_inserter
...
void test11() {
    ...
    if(it != vec1.end()) {
        ...
        std::cout << "Sotty, 5 not found" << std::endl;
    }
    display(vec1);
}
