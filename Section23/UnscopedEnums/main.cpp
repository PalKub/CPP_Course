...
void display_grocery_list(const std::vector<Grocery_Item> &grocery_list) {
    ...
    for (Grocery_Item grocery_item : grocery_list) {
        std::cout << grocery_item << std::endl; //Uses the overloded operator <<
...
//Using the unscoped enumeration to model grocery items
void test2() {
    std::cout << "\n--- Test2 -----------------------" << std::endl;
...
//Used for test3
//Unscoped enumerations holding the possible state
//and sequences of a rocket launch.
//Note the addition og the Unknown enumerator for the State enumeration.
...
//Used for test3
//Overloading the stream insertion oeprator to insert
...
void initiate(Sequence sequence) {
    std::cout << "Initiate " << sequence << " sequence!" << std::endl;  //Uses the overloaded operator<<
}
...
void test3() {
    ...
    std::cin >> state;  //Uses the oveloaded operator>>
