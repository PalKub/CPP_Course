...
//Capture by value
//The global variable global_x cannott be captured because it is not
...
//The captured variable x can only be modified withink the lambda by
//using the mutable keyword. Note that this modifiability only peratins to
...
void test4() {
    ...
    int z {300};    //Note that z is not captured in the lambda since it is not used/
                    //This should produce 'unused variable' warning when compiling.
...
//This time beacause all three variables - x, y, z are used within
...
public:
    ...
    //Note that the lambda in this example captures this by reference and ny value
