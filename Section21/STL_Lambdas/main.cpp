...
//Traingles are defined by a vector of three points in any order.
...
//each element for which the predicate returns false. The function the returns an
...
//sort - Sorting
//Sorts the elements in the sequence according to the predicate lambda.
...
//The same can be accomplished by using the std function "none_off" and
...
void test7() {
    ...
    if(pv1.is_valid(password))
        ...
        std::cout << "The password " << password << " is not walid!" << std::endl;
